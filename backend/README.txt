...
a backend under node.js with MySQL type of database.
...
| |_ js
| | |_ jquery.simple-mvvm.js  Script developed by the Beyond team to easily benefit an mvvm software architecture with only jquery.
| | |_ script.js              File script developed by the Beyond team and may be amended or supplemented by you.
...
| |_ layout                   When rendering a view, a layout is applied automagically (default main.ejs)
...
| | |_ main.ejs               This file puts your page views (eg index) by inserting the header and footer.
| |_ site
...
2. Install the SQL Script into your database server (see backend/data/mysql-data.sql).
3. Update the config file to access your database (see backend/config/main.js)
4. To start your server, use the command line:
   - with "node"   : node server.js port=8080
   - with "forever": forever start server.js port=8080
5. Launch your browser and to to URL: http://localhost:8080
6. Enjoy !!!!
