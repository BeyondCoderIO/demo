...
$(document).ready(function () {
    jQueryMVVM = new JQueryMVVM();
    var _ng_ifs = [];

    function JQueryMVVM() {
        this.$scope = undefined;
    }
    JQueryMVVM.prototype.then = function (callback) {
        callback(this.$scope);
    };
    JQueryMVVM.prototype.init = function (callback) {
        ...
        this.$scope = $scope.toJSON();
        $("[ng-init]").each(function() {
        ...
        $("[ng-click]").each(function() {
            var exp = $(this).attr('ng-click'),
                self = this;
            $(self).click(function() {
                jQueryMVVM.eval(exp, $(self));
            });
        });
        $("[ng-change]").each(function () {
            var self = this;
            $(self).change(function () {
                jQueryMVVM.eval($(self).attr('ng-change'), $(self));
            });
        });
        ...
        $("[ng-checked]").each(function () {
            var self = this,
                exp = $(this).attr('ng-checked');

            _ng_ifs.push({
                exp: function() {
                    if ($(self).prop("checked") != !!jQueryMVVM.eval(exp, $(self))) $(self).click();
                }
            });
        });
        $scope.setAll();
    ...
    JQueryMVVM.prototype.eval = function(str, elt) {
    ...
    function evaluateAll() {
        _ng_ifs.forEach(function(ng_if) {
            ...
            var show = jQueryMVVM.eval(ng_if.exp, ng_if.content);
    ...
    function NgListener(scopeNode, domElt) {
        ...
        $(this.domElt).change(function () {
            self.scopeNode.setValue(self.getValueFromHtml());
        });
